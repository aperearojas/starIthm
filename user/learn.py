...
if len(argv) > 10:
    ...
    simulations_filename = os.path.join(cwd_no_usrnm + '/tools/forward/simulations.dat')
...
plot_dir = os.path.join('product', 'plots-'+bname)
cov_dir = os.path.join('product', 'covs-'+bname)
table_dir = os.path.join('product', 'tables-'+bname)
perturb_dir = 'perturb/data'
...
if not os.path.exists('filetool/'):
    os.makedirs('filetool/')

if use_pickle and not os.path.exists(pickle_dir):
    os.makedirs(pickle_dir)

sys.stdout = open('filetool/product.txt', 'w+')
    ...
    for n_trees in [128]:
        forest = Pipeline(steps=[
            ('forest', ExtraTreesRegressor(
                ...
                n_estimators=n_trees,
                n_jobs=1,#min(n_trees, 2),
                oob_score=True, bootstrap=True))])
        start = time()
        forest.fit(X, ys)#new_ys)
        end = time()
        print "Number of trees:", n_trees, forest.steps[0][1].oob_score_
...
def plot_star(star, predict, y_names, out_dir=plot_dir):
    ...
    for ext in ['.png', '.pdf']:
        ...
        for (pred_j) in enumerate(y_names[0:num_ys]):
            (m, s) = (middles[pred_j], stds[pred_j])
            ...
            if num_ys%2==0 or num_ys%3==0 or int(sqrt)==sqrt:
                ...
                ax = plt.subplot2grid((rows, cols), (pred_j//2, (pred_j%2)*2), colspan=2)

            n, bins = ax.hist(predict[:,pred_j], 50, normed=1,
            ...
            ymin = ax.get_yaxis().get_view_interval()
        ...
        plt.savefig(os.path.join(out_dir, star + ext), dpi=400)
        plt.close()
...
def process_dir(directory=perturb_dir, perturb_pattern=perturb_pattern):
    ...
    table_curr.close()
    table_init.close()
    print "\nprediction time:", sum(run_times)
    print "time per star:", np.mean(run_times), "+/-", np.std(run_times)
    #sum(run_times)/len(run_times))
    print "time per perturbation:", np.mean(np.array(run_times) / 10000), "+/-", np.std(np.array(run_times) / 10000)
